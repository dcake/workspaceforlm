...
<script type="text/javascript">
	...
	function doSubmit(status) {
		...
		if (checkRemark.length > 200) {
			layer.alert('请输入小于200个字符', {
				icon : 3
			});
			return;
		}
		if (checkValue(checkRemark)) {
			layer.alert('您输入的审核意见包含非法内容，请进行修改！', {
				icon : 3
			});
			return;
		}
				...
				<form id="infoForm" method='post' class="breadcrumb form-search" style="background-color: white;" enctype="multipart/form-data">
					...
					<ul id="dataShowUl" class="ul-form">
						...
						<li><label>类型（保险）：</label> <input id="insuranceType" name="insuranceType" type="text" value="${goods.insuranceType }" class="input-medium"
							style="width:163px" readonly="readonly" /></li>
						<li><label>需求车型：</label> <input type="text" id="needTruckType" name="needTruckType" value="${goods.needTruckType }" class="input-medium"
							style="width:163px" readonly="readonly" /></li>
						<li class="clearfix"></li>
						...
						<li style="height:100px;width:100%;"><label style="float:left">审核意见：</label> <textarea cols='5' name="checkRemark" id="checkRemark"
								maxlength="100" style="width:480px;height:50px;float:left"></textarea></li>
