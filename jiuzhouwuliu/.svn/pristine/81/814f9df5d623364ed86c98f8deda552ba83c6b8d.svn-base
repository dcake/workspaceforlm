...
<script type="text/javascript">
	...
	function doSubmit() {
		...
		var options = {
			url : '${ctx}/driver/driveFleetList/updateFleet',
			type : 'post',
			traditional : true,
			data : {
				ids : ids.join(",")
		...
		<div class="tab-content">
			<div class="tab-pane active" id="home">
				<form id="infoForm" method='post' class="breadcrumb form-search" style="background-color: white;" enctype="multipart/form-data">
					<input type="text" id="id" name="id" value="${drivers.id}" style="display:none" /> <input type="text" id="userno" name="userno"
						value="${drivers.userno}" style="display:none" /> <input type="text" id="driveFleetId" name="driveFleetId" value="${drivers.driveFleetId}"
						style="display:none" />
					<ul class="ul-form">
						<li>
						...
						<li class="clearfix" style="margin-bottom:5px;">
							<div style="folat:right;text-align:right;">
								<input id="masterId" type="text" value="" placeHolder="请输入车队长ID" maxlength="100" class="input-medium" style="width:145px" />&nbsp;&nbsp; <input
									class="btn btn-primary" type="button" value="搜索" onclick="getDriverMaster();" />
							</div>
						</li>
						...
						<li class="clearfix" style="margin-bottom:5px;">
							<div style="folat:right;text-align:right;">
								<input type="text" id="memberId" value="" placeHolder="请输入车队员ID" maxlength="100" class="input-medium" style="width:145px" />&nbsp;&nbsp; <input
									class="btn btn-primary" id="searchMember" type="button" value="搜索" onclick="getDriverMember();" />
							</div>
						</li>
					</ul>
					<table id="memberInfo" style="margin-top:5px;width:100%;">
						<c:forEach items="${ds }" var="d">
							<tr id="${d.id}" userno="${d.userno}">
								...
								<td><a href="javascript:removeTr('${d.id }')">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</form>
