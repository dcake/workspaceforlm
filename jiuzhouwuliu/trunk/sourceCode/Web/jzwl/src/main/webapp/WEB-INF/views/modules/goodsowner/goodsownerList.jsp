...
<script type="text/javascript">
	...
	//1.货主详情
	function goGoodsownerDetail(id) {
		var url = "${ctx}/goodsowner/goodsowner/goGoodsownerDetail?id=" + id;
		openDialogViewForm('货主详情', url, '700px', '500px');
	}
	//2.打开认证信息对话框-----添加信息
	...
	//2.认证信息------添加信息
	function add() {
		var url = "${ctx}/goodsowner/goodsowner/add";
		openApproveInfoForm('添加货主', url, '700px', '500px');
	}
	//3.订单明细
	function order(id) {
		window.location.href = "${ctx}/goodsowner/goodsowner/order?id=" + id;
	}
	...
	function lock(id) {
		...
		//按钮
			...
			$.ajax({
				url : "${ctx}/goodsowner/goodsowner/lock",
				type : "post",
				data : {
					id : id
				},
	...
	function lock1(id) {
		...
		//按钮
			...
			$.ajax({
				url : "${ctx}/goodsowner/goodsowner/clear",
				type : "post",
				data : {
					id : id
				},
	...
	//4.认证信息------编辑信息
	function edit(id) {
		var url = "${ctx}/goodsowner/goodsowner/edit?id=" + id;
		openInfoForm('修改货主', url, '700px', '500px');
	}
</script>
<style>
ul {
...
<body>
	<form:form id="searchForm" modelAttribute="hotEvent" action="${ctx}/goodsowner/goodsowner/list" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			<li><label width="90px;">姓名：</label> <input id="truename" name="truename" type="text" maxlength="100" style="height:20px;"
				class="input-medium input-short" value="${goodsowners.truename}" /></li>
			<li><label width="90px;">手机号：</label> <input id="phoneno" name="phoneno" type="text" maxlength="11" style="height:20px;"
				class="input-medium input-short" value="${goodsowners.phoneno}" /></li>
			<li>
			<li><label>审核状态：</label> <select id="examineStatus" name="examineStatus" class="input-medium" style="width: 177px;">
					<option value="">不限</option>
					<c:forEach items="${goodsownersStatus}" var="t">
						<option value="${t.value}" <c:if test="${t.value==goodsowners.examineStatus}">selected</c:if>>${t.label}</option>
					</c:forEach>
			</select></li>
			<li><label>等级：</label> <select id="level" name="level" class="input-medium" style="width: 177px;">
					<option value="">不限</option>
					<c:forEach items="${goodsLevel}" var="t">
						<option value="${t.value}" <c:if test="${t.value==goodsowners.level}">selected</c:if>>${t.label}</option>
					</c:forEach>
			</select></li>
				...
				<c:if test="${not empty page.list }">
					<c:forEach items="${page.list}" var="s">
						<tr>
							...
							<td><c:if test="${s.level=='A'}">A </c:if> <c:if test="${s.level=='B'}">B</c:if> <c:if test="${s.level=='C'}">C</c:if></td>
							<td><a href="javascript:void(0);" onclick="goGoodsownerDetail('${s.id}');">详情</a> <a href="javascript:void(0);"
								onclick="edit('${s.id}');">编辑</a> <c:if test="${s.status=='0'}">
