		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
    				...
    				<c:choose>
    					<c:when test="${user.loginFlag=='0' || user.loginFailCount>3 }">
							<a href="${ctx}/sys/user/unlock?id=${user.id}" onclick="return confirmx('确认要解锁该用户吗？', this.href)">解锁</a>
    					</c:when>
    					<c:otherwise>
							<a href="${ctx}/sys/user/lock?id=${user.id}" onclick="return confirmx('确认要锁定该用户吗？', this.href)">锁定</a>
    					</c:otherwise>
    				</c:choose>
