...
public class GoodsownerService {
	...
	public AjaxBeanUtil saveGoodsowners(Users u,HttpServletRequest request){
		if(StringUtils.isEmpty(u.getId())||"undefined".equals(u.getId())){
			...
			goodsowners.setIdentityType(identityType);//个人身份选择（0个人、1公司)
			goodsowners.setIsTruecommpany("0");//是否企业认证（0否 1是）
			goodsowners.preInsert();
			// 将上传结果赋值到对象
			if (null != fileList) {
				SpringMvcUploadUtil.setObjectPropertyAndDelOldFile(fileList, goodsowners);
			}
